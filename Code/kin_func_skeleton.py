...
def rotation_3d(omega, theta):
    ...
    rot = I + omega_hat/omega_norm * np.sin(omega_norm * theta) + np.dot(omega_hat, omega_hat)/(omega_norm*omega_norm)*(1 - np.cos(omega_norm*theta))
...
def hat_3d(xi):
    ...
    #YOUR CODE HERE
    """
	xi[0] = v_x						[0   -w_z  w_y  v_x]
	xi[1] = v_y						[w_z  0   -w_x  v_y]
	xi[2] = v_z			xi_hat = 	[-w_y w_x  0    v_z]
	xi[3] = w_x						[0    0    0    0  ]
	xi[4] = w_y
	xi[5] = w_z
    """
...
def homog_2d(xi, theta):
	...
	xi[2] = w
    """
    rot = rotation_2d(xi[2] * theta)
    p = np.dot(np.identity(2)-rot, np.array([[-xi[1]/xi[2]], [xi[0]/xi[2]]]))
    g = np.identity(3)
    g[0:2, 0:2] = rot
    g[0:2, 2] = p[:,0]
...
def homog_3d(xi, theta):
    ...
    w = xi[3:6]
    omega_hat = skew_3d(w)
    e = sp.linalg.expm(omega_hat*theta)
    omega_norm = np.linalg.norm(w)
     
    g = np.identity(4)
    g[0:3,0:3] = e
    g[0:3,3] = 1/(omega_norm*omega_norm)*(np.dot((np.identity(3) - e),(np.dot(omega_hat,v))) + np.dot(np.outer(w,w),v*theta))
...
def prod_exp(xi, theta):
    ...
    g = np.identity(4)
    for i in range(N):
        e = sp.linalg.expm(hat_3d(xi[:,i]) * theta[i])
        g = np.dot(g, e)
